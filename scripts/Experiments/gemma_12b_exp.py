    ...
    for idx, img_path in enumerate(img_files):
        try:
            ...
            if json_result:
                ...
                print(f"[{processed_count + 1}/{total_images}] Processed {img_filename}: Emotion={emotion}") # just to track
            else:
                print(f"Failed to extract  {img_filename}")
                print(f"Raw output: {decoded_output[:200]}...")  #  first 200 chars for debugging
            ...
            if batch_counter >= batch_size:
                ...
                print(f"Checkpoint saved. Total processed so far are: {processed_count}/{total_images}")
                batch_counter = 0
